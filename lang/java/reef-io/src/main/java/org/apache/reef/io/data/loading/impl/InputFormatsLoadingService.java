 ...
 */
@DriverSide
public class InputFormatsLoadingService<K, V> implements DataLoadingService {

  private static final Logger LOG = Logger.getLogger(InputFormatsLoadingService.class.getName());
  ...
  @Inject
  public InputFormatsLoadingService(
      final InputFormats inputFormats,
      final JobConfs jobConf,
      ...
      @Parameter(JobConfsExternalConstructor.InputPaths.class) final Set<String> inputPaths) {

    this.inMemory = inMemory;
    this.inputFormatClass = inputFormatClass;
    this.inputPaths = inputPaths;
    this.evaluatorToPartitionMapper = null;
    this.numberOfPartitions = 0;

    LOG.info("***** Constructedddddddddddd ****");
  ...
  public Configuration getServiceConfiguration(final AllocatedEvaluator allocatedEvaluator) {
    ...
    try {
      ...
      return Tang.Factory.getTang().newConfigurationBuilder(serviceConfiguration)
          ...
          .bindNamedParameter(JobConfExternalConstructor.InputPath.class, inputPaths.iterator().next())
          .bindNamedParameter(
